...
readonly POSTGRES_EXPOSE_PORT_VALUE="5432"
...
write_env_file() {
...

  {
    ...
    printf 'POSTGRES_EXPOSE_PORT=%s\n' "$POSTGRES_EXPOSE_PORT_VALUE"
    ...
    printf 'BUILD_DATABASE_URL=postgresql://%s:%s@host.docker.internal:%s/%s\n' "$LOCAL_POSTGRES_USER_VALUE" "$LOCAL_POSTGRES_PASSWORD_VALUE" "$POSTGRES_EXPOSE_PORT_VALUE" "$LOCAL_POSTGRES_DB_VALUE"
    printf '\n'
